...
namespace UIVA_Server      // Unity Indie VRPN Adapter
{
    class UIVA
    {
        ...
        private int oculusriftsAlive = 0;
        ...
        private List<UIVA_OculusRift> oculusrifts = new List<UIVA_OculusRift>();
                ...
                if (line.Length != 0 && line[0] != '#')
                {
                    ...
                    switch (tokens[0])
                    {
                        ...
                        case "DEV_PHASESPACE":
                            ...
                            break;
                        case "DEV_OCULUSRIFT":
                            oculusrifts.Add(new UIVA_OculusRift(name));
                            oculusriftsAlive++;
                            Console.WriteLine("^_^ < VRPN > --- OculusRift # {0} tracking started...", oculusriftsAlive);
                            break;
                        default:
                ...
                else
                {
                    try
                    {
                        ...
                        switch (tokens[0])
                        {
                            ...
                            case "OculusRift":
                                if (which <= oculusriftsAlive)
                                {
                                    // Encode data of the requested LED in a message,
                                    // as specified in tokens[1]
                                    oculusrifts[which - 1].Update();
                                    sendStr = oculusrifts[which - 1].Encode();
                                }
                                else
                                {
                                    sendStr = "ERROR: OculusRift # " + which.ToString() + " is not turned on\n";
                                }
                                break;
